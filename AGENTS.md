...
5. Prefer `python -m safe_desk.web` for a normal trader (no terminal after start). CLI (`python -m safe_desk`) still does SMA/ATR/sizing, analog proof, and policy. No API secrets. Live numbers come from official MCP tool results (`--price-json` / `--balance-json`), never from a REST key.
