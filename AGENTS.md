...
5. Prefer `python -m safe_desk.web` for a normal trader (no terminal after start). CLI (`python -m safe_desk`) still does SMA/ATR/sizing, analog proof, and policy. No API secrets. Analyze may pull public MCP-shaped ticker/klines (no keys). Account/trade numbers come from official MCP tool results (`--price-json` / `--balance-json`). Never place via REST.
6. Demo files under `demo/` and the paper journal are **SIMULATED / PAPER**. Do not present them as live PnL.
