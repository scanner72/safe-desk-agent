...
4. **Policy** — allowlist, max notional, 1% risk, **stop-vs-risk** (same qty + farther stop → `STOP_RISK`), daily caps, emergency stop. Withdrawals always fail. Fail → ticket `BLOCKED`.
